...
class InventoryModel():
    ...
    def __init__(self, data_path, sku_col, time_col, loc_col, qty_col, prod_col, service_level):
        ...
        self.writer = pd.ExcelWriter('yourfile.xlsx')
        self.data.to_excel(self.writer,sheetname="Demand")
         # privremeno
    ...
    def define_subsets(self):
        ...
        self.intsku_fact = dict(zip(k.index, k))
    ...
    def define_paramaters(self, ftl_matrix):
        ...
        k = (k[k[self.sku_col].isin(self.int_skus)]
                              .groupby("Location")[self.sku_col]
                              .apply(set).apply(list))
        self.intsku_CW = dict(zip(k.index, k))
        ...
        self.cw_ss, self.rw_ss = cw_ss["Safety_Stock"].fillna(0).to_dict(), rw_ss["Safety_Stock"].fillna(0).to_dict()
    ...
    def define_objective(self):
        ...
        production_costs = LpAffineExpression(((self.production[i][f][t], self.loc_da["Prod. Costs"][f])
                                                ...
                                                for t in self.time_id))
        shortage_costs = LpAffineExpression((self.lost_sales[i][w][t], 50)
                                            for i in self.sku_id
                                            for w in self.loc_id
                                            for t in self.time_id)
    ...
    def define_constraints(self):
        ...
        #                                                           for x in sku_LOC.get(w, []))) <= self.loc_da["Hold. Cap."][w] 
        #                                                             for w in self.loc_id 
        #                                                             for t in self.time_id}) # limit to relvant subset
        ...
        constr_dic.update({f"{f,t, i}1stech_InvBal":LpAffineExpression(((self.production[i][f][lt[i,t]], 1),
                                                                        ...
                                                                        *((self.shipment[f][w][i][t], -1) for w in self.wh_id)))
                                                                        ...
                                                                        for t in self.time_id if self.demand.get((t,i,f), 0)})
        # constr_dic.update({f"{f,t, i}1stech_InvBal":LpAffineExpression(((self.production[i][f][lt[i,t]], 1),
        #                                                                 *((self.shipment[f][dc][i][t], -1) for dc in self.dc_id),
        #                                                                 *((self.shipment[f][w][i][t], -1) for w in self.wh_id)))
        #                                                                 >= self.inv_level[i][f][t] + self.demand.get((t,i,f), 0) - self.inv_level[i][f][prevt[t]]*inital_inv[t]
        #                                                                 for f in self.factory_id
        #                                                                 for i in self.intsku_fact.get(f, [])
        #                                                                 for t in self.time_id if self.demand.get((t,i,f), 0) == 0})
        ...
        constr_dic.update({f"{d,t, i}2ndech_InvBal":LpAffineExpression((*((self.shipment[f][d][i][prevt[t]], 1) for f in self.sku_plan[i]),
                                                                        *((self.shipment[d][w][i][t], -1) for w in self.wh_id if d in cw_to_rw[w])))
                                                                        == self.inv_level[i][d][t] + self.demand.get((t,i,d), 0) + self.cw_ss.get((d, i), 0) - self.lost_sales[i][d][t]- self.inv_level[i][d][prevt[t]]*inital_inv[t]
                                                                        for d in self.dc_id                 
        ...
        constr_dic.update({f"{w, t, i}3rdech_InvBal":LpAffineExpression((*((self.shipment[f][w][i][prevt[t]], 1) for f in factory_to_rw[w] if i in self.intsku_fact.get(f, [])),
                                                                        *((self.shipment[d][w][i][prevt[t]], 1) for d in cw_to_rw[w])))  
                                                                         == self.inv_level[i][w][t] + self.demand.get((t,i,w), 0) + self.rw_ss.get((w,i), 0) - self.lost_sales[i][w][t] - self.inv_level[i][w][prevt[t]]*inital_inv[t]
                                                                            for w in self.wh_id
        ...
        constr_dic.update({f"{d,t,i}2ndech_ext_sku_InvBal": LpAffineExpression((*((self.shipment[e][d][i][prevt[t]], 1) for e in self.supplier[i]),
                                                                               *((self.shipment[d][w][i][t], -1) for w in self.wh_id if d in cw_to_rw[w])))
                                                                        == self.demand.get((t,i,d), 0) + self.cw_ss.get((d, i), 0) - self.lost_sales[i][d][t] + self.inv_level[i][d][t] - self.inv_level[i][d][prevt[t]]*inital_inv[t]
                                                                        for d in self.dc_id
        ...
        constr_dic.update({f"{d, t, i}2ndech_ssreq":LpAffineExpression([(self.inv_level[i][d][t], 1)]) 
                                                                            >= self.cw_ss.get((d, i), 0) - self.lost_sales[i][d][t]
                                                                            for d in self.dc_id
                                                                            for i in self.intsku_CW.get(d, []) + self.extsku_CW.get(d, [])
                                                                            for t in self.time_id})

        constr_dic.update({f"{w, t, i}3rdech_ssreq":LpAffineExpression([(self.inv_level[i][w][t], 1)]) 
                                                                        >= self.rw_ss.get((w,i), 0) - self.lost_sales[i][w][t]
                                                                        for w in self.wh_id
                                                                        for i in self.intsku_RW.get(w, []) + self.extsku_RW.get(w, [])
                                                                        for t in self.time_id})
        constr_dic.update({f"{w, t, i}LS":LpAffineExpression([(self.lost_sales[i][w][t], 1)]) 
                                                                    == 0
                                                                    for w in self.loc_id
                                                                    for i in self.sku_id
                                                                    for t in self.time_id if self.demand.get((t,i,w), 0) == 0})
        constr_dic.update({f"{w, t, i}LS":LpAffineExpression([(self.lost_sales[i][w][t], 1)]) 
                                                                    <= self.demand.get((t,i,w), 0) 
                                                                    for w in self.loc_id
                                                                    for i in self.sku_id
                                                                    for t in self.time_id if self.demand.get((t,i,w), 0) > 0})
    ...
    def direct_sh_constraint(self):
        ...
        constr_dic.update({f"{w,t,i}3rdech_ext_sku_InvBal": LpAffineExpression([*((self.shipment[e][w][i][prevt[t]], 1) for e in self.supplier[i])])
                                                                    == self.demand.get((t,i,w), 0) + self.rw_ss.get((w,i), 0) - self.lost_sales[i][w][t] + self.inv_level[i][w][t] - self.inv_level[i][w][prevt[t]]*inital_inv[t]
                                                                    for w in self.wh_id if direct_sh_extsku[w] == 1
        ...
        constr_dic.update({f"{w,t,i}3rdech_ext_sku_InvBal": LpAffineExpression([*((self.shipment[d][w][i][prevt[t]], 1) for d in cw_to_rw[w])])
                                                                    == self.demand.get((t,i,w), 0) + self.rw_ss.get((w,i), 0) - self.lost_sales[i][w][t] + self.inv_level[i][w][t] - self.inv_level[i][w][prevt[t]]*inital_inv[t]
                                                                    for w in self.wh_id if direct_sh_extsku[w] == 0
                                                                    for i in self.extsku_RW.get(w, [])
                                                                    for t in self.time_id})
        self.inv_model.extend(constr_dic)
    ...
    def min_batch_size_constraint(self, path):
        constr_dic = {}
        sku_min_batch = []
        f_active = dict(zip(self.loc_data.index, self.loc_data["Min. Batch"]))
        m = pd.read_csv(path,
                        index_col=[0])
        min_batch_size = dict(zip(m.index, m["Min. Batch Size (PAL)"]))
        for f in self.factory_id:
            if f_active[f] == 1:
                sku_min_batch += self.intsku_fact.get(f, [])
        self.prod_on = pulp.LpVariable.dicts("Binary min batch size",
                                             (sku_min_batch,
                                              self.time_id),
                                             cat="Binary")

        constr_dic.update({f"{i, t}Min_batch_size": LpAffineExpression([(self.production[i][f][t], 1)])
                                                                        >= self.prod_on[i][t] * min_batch_size[i]
                                                                        for i in sku_min_batch
                                                                        for f in self.factory_id if i in self.intsku_fact.get(f, [])
                                                                        for t in self.time_id})
        constr_dic.update({f"{i, t}Max_batch_size": LpAffineExpression([(self.production[i][f][t], 1)]) 
                                                                         <= self.prod_on[i][t] * 900
                                                                        for i in sku_min_batch
                                                                        for f in self.factory_id if i in self.intsku_fact.get(f, [])
                                                                        for t in self.time_id})
        self.inv_model.extend(constr_dic)
    ...
    def compute_ss(self, service_level):
        ...
        print(cw_ss["Safety_Stock"].max(),
              rw_ss["Safety_Stock"].max(),
              "------------------SS")
        return cw_ss, rw_ss
    ...
    def build_model(self):
        ...
        self.direct_sh_constraint()
        self.min_batch_size_constraint(path="./CSV input files/PPF Batch Size.csv")
    ...
    def bi_objective(self):
        ...
        self.inv_model.extend({"service_level_measure": LpAffineExpression((self.lost_sales[i][w][t], 1)
                                                                            for i in self.sku_id
                                                                            for w in self.loc_id
                                                                            for t in self.time_id) == (1- self.service_level) * self.total_demand})
        

        for i in range(5):
            # Solving for first objective
            f1 = self.define_objective()
            f2 = self.service_level
            self.inv_model += f1
            self.inv_model += f2 >= epsilon[-1]
            solver = CPLEX_PY()
            # self.inv_model.solve(solver)
            solver.buildSolverModel(self.inv_model)
            solver.solverModel.parameters.emphasis.memory.set(1)
            ...
            f1_value = value(self.inv_model.objective)
